...
contains
   ...
   subroutine solve_momentum(state, at_first_timestep, timestep)
	  ...
	  ! The divergence matrix (extracted from state)
	  ...
	  type(scalar_field), pointer :: p, density, pt
  type(scalar_field), pointer :: old_density
	  ...
	  real :: theta_pg, theta_u, theta_nl
  ...
  type(scalar_field) :: T
	  ...
	  integer :: stat, it, ntsol, f, nfields
	  ...
	  density => extract_scalar_field(state(1), "Density", stat)
	  p => extract_scalar_field(state(1), "Pressure", stat)
	  pt => extract_scalar_field(state(1), "PotentialTemperature", stat)
	  ...
	  assembly_loop: do istate = 1, size(state)
		 ...
		 if(.not. have_pressure) then
			...
			! to again decide reassemble_ct_m as ctp_m for this case is assembled when ct_m is.
				if (.not. (lcompressible_eos .or. shallow_water_projection) .and. cg_pressure_cv_test_continuity) then
			   if (implicit_prognostic_fs) then
		 ...
		 if (standard_fs .or. implicit_prognostic_fs .or. lcompressible_eos .or. shallow_water_projection) then
		...
		if((lcompressible_eos .or. shallow_water_projection) .and. multiphase .and. (use_theta_pg .or. use_theta_divergence)) then
		 ...
		 ! Assemble divergence matrix C^T: div(u)
	   ...
	   ! rho*div(u) + u*grad(rho) for the single compressible phase.
			...
			if (lcompressible_eos .or. shallow_water_projection .or. cg_pressure_cv_test_continuity) then
			   if (have_rotated_bcs(u)) then
				  if (dg(istate)) then
			...
			if(reassemble_cmc_m) then
		   		ewrite(2,*) "Reassemble CMC pressure matrix"
			   call zero(cmc_m)
	  ...
	  if (.not.reduced_model) then
		 ...
		 if(prognostic_p) then
			...
			call correct_pressure(state, prognostic_p_istate, x, u, p, old_p, delta_p, &
								 p_theta, free_surface, theta_pg, theta_divergence, cmc_m, &
...
!               end if
			...
			velocity_correction_loop: do istate = 1, size(state)
			   ...
			   if(have_option(trim(u%option_path)//"/prognostic/spatial_discretisation"//&
				  ...
				  if(lcompressible_eos .or. shallow_water_projection) then
   ...
   subroutine get_pressure_options(p)
	  ...
	  lcompressible_eos = option_count("/material_phase/equation_of_state/compressible") > 0
   ...
   subroutine advance_velocity(state, istate, x, u, p_theta, big_m, ct_m, mom_rhs, subcycle_m, subcycle_rhs, inverse_mass)
	  ...
	  type(scalar_field), pointer :: hb_pressure, hr_pressure, density
	  ...
	  if (associated(ct_m(istate)%ptr)) then
		 ...
		 ewrite(1,*) "shelf: Entering compute_pressure_and_tidal_gradient"
			...
			call allocate(hb_pressure,p_theta%mesh,"ReferencePressure")
			call zero(hb_pressure)
		hr_pressure => extract_scalar_field(state(istate), "HydrostaticReferencePressure", stat)
			call safe_set(state(istate),hb_pressure,hr_pressure)
			if(stat /= 0) then
	...
	!   ewrite_minmax(big_m(istate))
	  ewrite_minmax(mom_rhs(istate))
	  ewrite_minmax(delta_u)
  ...
  if((lcompressible_eos .and. have_option(trim(state(istate)%option_path)//'/equation_of_state/compressible')) &
   ...
   subroutine correct_pressure(state, prognostic_p_istate, x, u, p, old_p, delta_p, p_theta, &
							  free_surface, theta_pg, theta_divergence, &
